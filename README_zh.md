...
- **方向键交互系统**（v3.1.5+）：支持 whiptail/dialog 方向键选择、空格勾选、自动滚动列表，无依赖时自动降级到数字输入模式。
- **返回上一级功能**：在所有菜单中支持按 0 或 ESC 键返回上一级，提升导航体验。
- **全球榜单集成**（计划中）：未来版本将内置支持同步全球热门榜单（如"热门音频视频每日排行榜 - 全球"）。
